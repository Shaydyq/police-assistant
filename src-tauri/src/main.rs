...
async fn update_sources(state: &AppState, sources: &[String]) -> Result<()> {
    ...
    for url in sources {
        ...
        replace_document(&mut conn, &title, url, &text, &hash)?;
    }
    Ok(())
}
...
fn list_documents(state: State<'_, Mutex<AppState>>) -> CommandResult<Vec<DocumentSummary>> {
    ...
    for row in rows {
        docs.push(row.map_err(|err| err.to_string())?);
    }
    Ok(docs)
}
...
fn search_articles(
    ...
    let mut articles = Vec::new();
    for row in rows {
        articles.push(row.map_err(|err| err.to_string())?);
    }
    Ok(articles)
}
...
fn search_memos(state: State<'_, Mutex<AppState>>, query: String) -> CommandResult<Vec<MemoSummary>> {
    ...
    for row in rows {
        memos.push(row.map_err(|err| err.to_string())?);
    }
    Ok(memos)
}

#[tauri::command]
fn get_memo(state: State<'_, Mutex<AppState>>, memo_id: i64) -> CommandResult<MemoDetail> {
    let state = state.lock().expect("state lock");
    ...
    for row in rows {
        linked.push(row.map_err(|err| err.to_string())?);
    }
...
fn assistant_answer(
    ...
    for article in articles {
        let (id, number, title, body) = article.map_err(|err| err.to_string())?;
    ...
    for memo in memos {
        let (id, title, body) = memo.map_err(|err| err.to_string())?;
